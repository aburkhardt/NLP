...
def ngram_dict(data, ngrams = "tag_word"):
    ...
    Arguments:
        ...
        ngrams: denote type of ngram (unigram or bigram) and if want words or tags: word_word or tag_word
    Returns:
...
def compute_transition_matrix (tags, bigram_counts, unigram_counts, k):
    ...
    Arguments:
        ...
        bigram_counts: count of bigrams of OBI tags in training data (used for numerator)
        unigram_counts: count of unigram OBI tag in training data (used for denominator)
        k: +k smoothing value

    Returns: 4x3 matrix of transition probabilities for all possible POS tags
    ...
    for x in tags:
            pair = ('',x) # here the period denotes the start of a sentence.
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("Usage: burkhardt-amy-assgn4.py trainingfile.txt testfile.txt > burkhardt-amy-assgn4-test-output.txt")
